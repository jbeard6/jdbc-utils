...
/**
 * A function that can throw a {@link SQLException}.
 */
@FunctionalInterface
public interface JdbcFunction<T, R> {
     ...
     *             if before is null
     *
     * @see #andThen(JdbcFunction)
     */
    default <V> JdbcFunction<V, R> compose(JdbcFunction<? super V, ? extends T> before) {
     ...
     *             if after is null
     *
     * @see #compose(JdbcFunction)
     */
    default <V> JdbcFunction<T, V> andThen(JdbcFunction<? super R, ? extends V> after) {
     ...
     */
    static <T> JdbcFunction<T, T> identity() {
        return t -> t;
    }
